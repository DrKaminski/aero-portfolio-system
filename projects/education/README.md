# 1. Educational Background with PhD 🚀

## Duke University - Ph.D. in Mechanical Engineering
**August 2022 – Present** | Durham, NC

**GPA**: 3.8

**Tentative Dissertation**: Global Stability Analysis for Nonlinear Flow Instabilities via Harmonic Balance Techniques

### Key Research (Aeroelasticity Group)
- Developing harmonic balance numerical solvers for non-synchronous vibrations in turbomachinery with Dr. Kenneth Hall.
- In-house Fortran code for novel fluid-structure interaction models.
- Directly applicable to Raptor engine stability and Starship reentry aeroelastic loads.

**M.S. Mechanical Engineering** (University of Central Florida) & **B.S. Aerospace Engineering** (UCF, Honors)

**SpaceX Relevance**: Advanced nonlinear stability analysis techniques for propulsion and vehicle dynamics.

**Artifacts**: [Fortran Solver Code](./code/) | [Stability Plots](./assets/)

[← Back to Mission Control](../README.md)